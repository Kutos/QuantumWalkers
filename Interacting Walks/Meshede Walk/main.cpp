...
#pragma endregion
	...
	tempState[N / 2][N / 2][1] = complex(1.0 / sqrt(4.0), 0.0);
	tempState[N / 2][N / 2][2] = complex(-1.0 / sqrt(2.0), 0.0);
	...
	for (unsigned int i = 1; i < N; i += 2)
	{
			outFile << tempState[i][i][0].abs2() + tempState[i][i][1].abs2() + tempState[i][i][2].abs2() + tempState[i][i][3].abs2() << " ";
		outFile << std::endl;
	}
	outFile.close();
	delete[] tempState;
